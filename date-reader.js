...
(define (parse-date date . rest)
  (let ((slots (if (>= (length rest) 2) (second rest) (map (lambda (x) (list (second x))) *date-symbols*)))
        (formats (if (not (empty? rest)) (car rest) (map second *date-formats*))))
    (if (empty? formats)
        slots
        (let ((r (parse-date-format date (car formats) slots #t)))
          (if r (set! slots r))
          (parse-date date (cdr formats) slots #t)))))
  ...
  (cond ((empty? format) slots)
        ...
        ((string? (car format))
         (if (string-prefix-ci? (car format) date)
             (parse-date-format (after-string (car format) date) (cdr format) slots #f)
             #f))
        (#t
         (if (not (empty? (cdr (assoc (car format) slots))))
             #f
             (let* ((r (string-search (multiline "^" (lookup-date-regex (car format))) date)))
               (if (not r)
                   #f
                   (parse-date-format (after-regex (multiline "^" (lookup-date-regex (car format))) date)
                                      (cdr format)
                                      (cons (cons (car format) (second r)) slots)
                                      #f)))))))
(define (build-format-regex lis)
  (apply string-append (map (lambda (x) (if (string? x)
                                            (regexp-escape x)
                                            (lookup-date-regex x)))
                            lis)))

(define (lookup-date-regex x) (second (assoc x (map cdr *date-symbols*))))
